...
const API_NEW_EXPENSE = environment.apiBaseLink + '/api/parking/';
...
export class ParkingService {
  ...
  addParking(data: Parking): Observable<ResponsePayload> {
    return this.httpClient.post<ResponsePayload>(API_NEW_EXPENSE + 'add', data);
  }

  getAllParking(filterData: FilterData, searchQuery?: string) {
  ...
  getParkingById(id: string, select?: string) {
    let params = new HttpParams();
    if (select) {
  ...
  updateParkingById(id: string, data: Parking) {
  ...
  deleteParkingById(id: string, checkUsage?: boolean) {
    let params = new HttpParams();
    if (checkUsage) {
  ...
  deleteMultipleParkingById(ids: string[], checkUsage?: boolean) {
    let params = new HttpParams();
    if (checkUsage) {
      params = params.append('checkUsage', checkUsage);
    }
  ...
  //  parkingGroupByField<T>(dataArray: T[], field: string): ParkingGroup[] {
